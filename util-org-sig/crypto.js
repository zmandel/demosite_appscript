...
/**
 * ensure privateKey and publicKey are CryptoKey.
 * converts them to CryptoKey if  they are plain objects
 */
async function ensureCryptoKeys() {
  // Import the private key if it's not a CryptoKey
  if (!(g_keyPair.privateKey instanceof CryptoKey)) {
    g_keyPair.privateKey = await importPrivateKey(g_keyPair.privateKey);
  }
  // Import the public key if it's not a CryptoKey
  if (!(g_keyPair.publicKey instanceof CryptoKey)) {
    g_keyPair.publicKey = await importPublicKey(g_keyPair.publicKey);
  }
}

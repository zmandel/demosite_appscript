...
/**
 * PUBLIC FUNCTIONS (callable from the client).
 * Only functions listed in PUBLIC_FUNCTIONS are allowed.
...
/**
 * processRequest: Parses the JSON payload, validates the function name,
 * ensures that the function is one of our public (whitelisted) functions,
 * and then calls it with the provided arguments.
 */
function processServerRequest(e) {
  try {
    var payload = {
    ...
    var result = func.apply(null, payload.arguments || []);
...
function doGet(e) {
  ...
  if (e.parameter) {
    if (e.parameter.page === "2")
      bPage2 = true;
    else if (e.parameter.bridge === "1") {
      let htmlOutput = HtmlService.createHtmlOutputFromFile("html/bridge");
